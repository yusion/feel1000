...
$(document).ready(function(){	
    $("#email").focus(function(){ //文本框获得焦点，插入Email提示层
        $("#emailTip").remove();
	$(this).after("<div id='emailTip' style='width:" + ($(this).width() + 17)+"px; height:auto; background:#fff; color:#6B6B6B; position:absolute; left:"+$(this).get(0).offsetLeft+"px; top:"+($(this).get(0).offsetTop+$(this).height()+13)+"px; border:1px solid #ccc;z-index:5px; '></div>");
        if($("#emailTip").html()){
             $("#emailTip").css("display","block");
	$(".newemail").css("width",$("#emailTip").width());
		can1press = true;
        } else {
		...
		if (press!="" || press!=null){
		var emailtxt = "";			
		var emailvar = new Array("@163.com","@126.com","@qq.com","@sina.com","@gmail.com","@hotmail.com","@foxmail.com","@yahoo.com");
		totalid = emailvar.length;
			...
			$("#emailTip").html(emailmy+emailtxt);
			if($("#emailTip").html()){
				 $("#emailTip").css("display","block");
				 $(".newemail").css("width",$("#emailTip").width());
				 can1press = true;
